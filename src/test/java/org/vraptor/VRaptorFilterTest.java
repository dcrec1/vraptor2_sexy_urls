...
public class VRaptorFilterTest extends AbstractTest {
	...
	public void testInvokesItsControllerExecution() throws ServletException,
			IOException {
	...
	public void testGivesA404IfLogicNotFound() throws ServletException,
			IOException {

		controllerMock.expects(once()).method("execute").will(
				throwException(new LogicNotFoundException("")));
		Mock response = mock(HttpServletResponse.class);
		response.expects(once()).method("sendError").with(eq(404),
				isA(String.class));
		filter.doFilter(null, (HttpServletResponse) response.proxy(),
				(FilterChain) chain.proxy());

	}

	public void testGivesA404IfComponentNotFound() throws ServletException,
			IOException {

		controllerMock.expects(once()).method("execute").will(
				throwException(new ComponentNotFoundException("")));
		Mock response = mock(HttpServletResponse.class);
		response.expects(once()).method("sendError").with(eq(404),
				isA(String.class));
	...
	protected void setUp() throws Exception {
		...
		config.expects(once()).method("getServletContext").will(
				returnValue(new MockedServletContext()));
